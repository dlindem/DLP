...
def get_token():
	...
	while True:
		try:
			login = site.login(username=wd_bot_user, password=wd_bot_pwd)
			break
...
# get Wikibase lexemes and senses' Wikidata alignments

url = "https://illlp.wikibase.cloud/query/sparql?format=json&query=PREFIX%20ilwb%3A%20%3Chttps%3A%2F%2Filllp.wikibase.cloud%2Fentity%2F%3E%0APREFIX%20ildp%3A%20%3Chttps%3A%2F%2Filllp.wikibase.cloud%2Fprop%2Fdirect%2F%3E%0APREFIX%20ilp%3A%20%3Chttps%3A%2F%2Filllp.wikibase.cloud%2Fprop%2F%3E%0APREFIX%20ilps%3A%20%3Chttps%3A%2F%2Filllp.wikibase.cloud%2Fprop%2Fstatement%2F%3E%0APREFIX%20ilpq%3A%20%3Chttps%3A%2F%2Filllp.wikibase.cloud%2Fprop%2Fqualifier%2F%3E%0APREFIX%20ilpr%3A%20%3Chttps%3A%2F%2Filllp.wikibase.cloud%2Fprop%2Freference%2F%3E%0APREFIX%20ilno%3A%20%3Chttps%3A%2F%2Filllp.wikibase.cloud%2Fprop%2Fnovalue%2F%3E%0A%0Aselect%20%3Fwb_entity%20%3Ftype%20%3Fwd_entity%20%3Fxml_id%20%3Fsource_date%20where%0A%0A%7B%0A%20%20%7B%20%3Fwb_entity%20dct%3Alanguage%20ildp%3AQ3%3B%20ildp%3AP1%20%3Fwd_entity%3B%20ilp%3AP6%20%5Bilps%3AP6%20%3Fxml_id%3B%20prov%3AwasDerivedFrom%20%5Bilpr%3AP12%20%3Fsource_date%5D%5D.%20bind%20(%22entry%22%20as%20%3Ftype)%7D%0A%20%20union%0A%20%20%7B%20%3Fwb_entity%20skos%3Adefinition%20%5B%5D%3B%20ildp%3AP1%20%3Fwd_entity%3B%20ilp%3AP6%20%5Bilps%3AP6%20%3Fxml_id%3B%20prov%3AwasDerivedFrom%20%5Bilpr%3AP12%20%3Fsource_date%5D%5D.%20bind%20(%22sense%22%20as%20%3Ftype)%7D%0A%20%20%20%0A%7D%20"
wb_r = requests.get(headers=headers, url=url)
print(wb_r)
...
for wb_id in wb_mappings.keys():
	...
	if count > 1:
		break
	print(f"\n[{count}/{len(wb_mappings)}] now processing https://illlp.wikibase.cloud/entity/{wb_id}.")
	wd_id = wb_mappings[wb_id]['wd_entity']['value']
	if wd_id not in wd_unaligned:
		print(f"{wd_id} not in list of unmapped Wikidata items.")
		continue
	...
	while (not claim_id):
		try:
			request = site.post('wbcreateclaim', token=token, entity=wd_id, property="P14752", snaktype="value", value=f'"{xml_id}"',
	...
	while True:
		try:
			setref = site.post('wbsetreference', token=token, statement=claim_id, index=0, snaks=refsnaks, bot=1, summary="DLP xml source date.")
			if setref['success'] == 1:
